...
export default function TransportAndamento(props) {
  ...
  const filtered = useMemo(() =>
    notes.filter(n => ANDAMENTO_STATUS.includes(getTracking(n, statuses)))
  , [notes, statuses]);
    ...
    return (
      <div style={{ textAlign: 'center', padding: '64px 24px' }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-3)', maxWidth: 300, margin: '0 auto' }}>
          Quando iniciar o processo de devolução, as notas aparecerão aqui.
        </div>
      </div>
    );
  }
  ...
  return (
    <NoteListView
      ...
      emptyMessage="Nenhuma nota em andamento."
    />
  );
}
