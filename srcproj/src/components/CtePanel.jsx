...
export default function CtePanel({ noteKey, extras = {} }) {
  ...
  useEffect(() => {
    // Primeiro tenta dos extras (já carregados em memória)
    const fromExtras = extras['cte:' + noteKey];
    if (fromExtras) {
      setCte(typeof fromExtras === 'object' ? fromExtras : null);
      setLoading(false);
      return;
    }
  ...
  if (!cte) return (
    <div style={{ padding: '24px 0', textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 11, color: 'var(--text-3)' }}>
        Quando a transportadora emitir o CT-e no Active onSupply, ele aparecerá aqui automaticamente.
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: 'rgba(63,185,80,.1)', border: '1px solid rgba(63,185,80,.25)', borderRadius: 10 }}>
        ...
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#3FB950' }}>CT-e emitido — Active onSupply</div>
          <div style={{ fontSize: 10, color: 'var(--text-3)', marginTop: 1 }}>Sincronizado em {dtFmt(cte.sincronizado_em)}</div>
        </div>
      </div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[
            { l: 'Transportadora', v: cte.transportador, bold: true },
            { l: 'Valor do frete', v: fmt(cte.valor_frete), color: '#3FB950', bold: true },
            { l: 'Emissão', v: dtFmt(cte.data_emissao) },
            { l: 'Previsão entrega', v: dtFmt(cte.data_previsao) },
            { l: 'Entrega realizada', v: dtFmt(cte.data_entrega) },
          ].map(d => d.v && d.v !== '—' ? (
          ...
          {cte.chave && (
            <div>
              <div style={{ fontSize: 10, color: 'var(--text-3)', marginBottom: 3 }}>Chave eletrônica</div>
              <div style={{ fontSize: 9, fontFamily: 'monospace', color: 'var(--text-2)', wordBreak: 'break-all', background: 'var(--surface-2)', padding: '6px 8px', borderRadius: 6 }}>
                {cte.chave}
              </div>
            </div>
          )}
