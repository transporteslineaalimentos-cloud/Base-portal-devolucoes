        ...
        return (
          <div key={i} style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start' }}>
            <div>
              ...
              <div className={`chat-bubble ${isMine ? 'mine' : 'theirs'}`}>{m.msg}</div>
            </div>
          </div>
        );
      })}
      <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
        <input value={msg} onChange={e => setMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && send()} placeholder="Escrever mensagem..." className="input" style={{ flex: 1, padding: '8px 12px' }} />
        <button onClick={send} className="btn btn-gold btn-sm" style={{ padding: '8px 12px' }}><SendIcon /></button>
      </div>
    </div>
  );
}
