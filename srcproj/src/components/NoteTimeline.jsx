          ...
          <div key={ev.id || i}>
            ...
            <div style={{ display: 'flex', gap: 14, marginBottom: 12, position: 'relative' }}>
              <div style={{ position: 'relative', zIndex: 1, flexShrink: 0 }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', background: c.bg, border: `2px solid ${c.dot}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: isLast ? 16 : 14, boxShadow: isLast ? `0 0 0 4px ${c.bg}` : 'none' }}>
                  {c.ic}
                </div>
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 14px', borderLeft: `3px solid ${c.dot}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: c.dot }}>{ev.action}</div>
                    <div style={{ fontSize: 10, color: 'var(--text-3)', whiteSpace: 'nowrap', marginLeft: 8 }}>{dateStr} · {timeStr}</div>
                  </div>
                  {ev.status_from && ev.status_to && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 11, color: 'var(--text-3)', background: 'var(--surface-2)', padding: '2px 8px', borderRadius: 4 }}>{ev.status_from}</span>
                      <span style={{ fontSize: 11, color: 'var(--text-3)' }}>→</span>
                      <span style={{ fontSize: 11, color: 'var(--text)', fontWeight: 600, background: `${c.dot}18`, padding: '2px 8px', borderRadius: 4, border: `1px solid ${c.dot}30` }}>{ev.status_to}</span>
                    </div>
                  )}
                  {ev.observation && (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div style={{ width: 20, height: 20, borderRadius: '50%', background: 'rgba(166,139,92,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#A68B5C', flexShrink: 0 }}>
