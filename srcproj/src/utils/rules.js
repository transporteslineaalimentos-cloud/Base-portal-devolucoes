...
export const RULES = [
  {
    nome: 'Área Transporte → Responsável',
    trigger: 'import',
    condition: (note) => note.ar === 'TRANSPORTE',
    action: () => ({ responsavel: 'equipe_transporte@linea.com' })
  },
  {
    nome: 'Retorno autorizado → Flag',
    trigger: 'tracking_change',
    condition: (note, payload) => payload?.newTracking === 'retorno_auto',
  ...
  {
    nome: 'Aging > 15 dias → Prioridade alta',
    trigger: 'daily',
    condition: (note) => (calcAging(note) || 0) > 15,
