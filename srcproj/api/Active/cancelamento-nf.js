  ...
  try {
    ...
    for (const item of items) {
      ...
      await sbInsert('active_webhooks', {
        ...
        observacao: g(item, 'OBSERVACAO', 'Observacao', 'MOTIVO_CANCELAMENTO', 'MOTIVO'),
        payload_raw: item,
      });
    ...
    return res.status(200).json(results.map((r, i) => ({
      ...
      Linha: i + 1, Tipo_Documento: 'Cancelamento NF',
      Referencia: `Cancelamento NF: ${r.numero}`, Campo_Relacionado: '',
      Erro: false, Informacao_Complementar: {},
    })));
  } catch (err) {
    try { await sbInsert('active_webhooks', { tipo: 'nf_cancelamento_erro', source: 'active_onsupply', payload_raw: req.body || 'empty', observacao: err.message }); } catch {}
    return res.status(200).json([{ Guid_Processamento: `error-${Date.now()}`, Chave_Cliente: 'LINEA_PORTAL', Inicio_Processamento: new Date().toISOString(), Termino_Processamento: new Date().toISOString(), Tempo_Processamento: '00:00:00.000', Codigo: '999', Mensagem: `Erro: ${err.message}`, Linha: 1, Tipo_Documento: 'Cancelamento NF', Referencia: '', Campo_Relacionado: '', Erro: true, Informacao_Complementar: {} }]);
  }
}
