  ...
  try {
    ...
    for (const item of items) {
      ...
      const row = {
        ...
        status_ocorrencia: 'entrega',
        observacao: g(ocorr, 'OBSERVACAO', 'Observacao', 'OBS') || g(item, 'OBSERVACAO', 'Observacao'),
        recebedor_nome: g(entrega, 'RECEBEDOR', 'NOME_RECEBEDOR') || g(item, 'RECEBEDOR', 'NOME_RECEBEDOR'),
        recebedor_documento: g(entrega, 'DOCUMENTO_RECEBEDOR', 'CPF_RECEBEDOR') || g(item, 'DOC_RECEBEDOR'),
        data_entrega: g(entrega, 'DATA', 'DATA_ENTREGA') || g(item, 'DATA_ENTREGA') || null,
    ...
    return res.status(200).json(results.map((r, i) => ({
      ...
      Linha: i + 1, Tipo_Documento: 'Ocorrencia Entrega',
      Referencia: `Entrega: ${r.numero}`, Campo_Relacionado: '',
      Erro: false, Informacao_Complementar: {},
    })));
  } catch (err) {
    try { await sbInsert('active_ocorrencias', { tipo: 'ocorrencia', subtipo: 'entrega_erro', source: 'active_onsupply', payload_raw: req.body || 'empty', observacao: err.message }); } catch {}
    return res.status(200).json([{ Guid_Processamento: `error-${Date.now()}`, Chave_Cliente: 'LINEA_PORTAL', Inicio_Processamento: new Date().toISOString(), Termino_Processamento: new Date().toISOString(), Tempo_Processamento: '00:00:00.000', Codigo: '999', Mensagem: `Erro: ${err.message}`, Linha: 1, Tipo_Documento: 'Ocorrencia Entrega', Referencia: '', Campo_Relacionado: '', Erro: true, Informacao_Complementar: {} }]);
  }
}
