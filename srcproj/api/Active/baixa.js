...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json(results.map((r, i) => ({
      ...
      Linha: i + 1, Tipo_Documento: 'Baixa Ocorrencia',
      Referencia: `Baixa: ${r.numero}`, Campo_Relacionado: '',
      Erro: false, Informacao_Complementar: {},
    })));
  } catch (err) {
    try { await sbInsert('active_ocorrencias', { tipo: 'ocorrencia', subtipo: 'baixa_erro', source: 'active_onsupply', payload_raw: req.body || 'empty', observacao: err.message }); } catch {}
    return res.status(200).json([{ Guid_Processamento: `error-${Date.now()}`, Chave_Cliente: 'LINEA_PORTAL', Inicio_Processamento: new Date().toISOString(), Termino_Processamento: new Date().toISOString(), Tempo_Processamento: '00:00:00.000', Codigo: '999', Mensagem: `Erro: ${err.message}`, Linha: 1, Tipo_Documento: 'Baixa Ocorrencia', Referencia: '', Campo_Relacionado: '', Erro: true, Informacao_Complementar: {} }]);
  }
}
